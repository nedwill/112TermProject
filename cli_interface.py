...
for task in tasks_input:
    ...
    if "recurring " == task[: len("recurring ")]:
        ...
        due_dt = datetime.date(
            *map(int, due.split("-"))
        )  # should handle year/day thing
        ...
        if "fixed " == desc[: len("fixed ")]:
            new_task = FixedTask(
                desc, int(hours), due_dt
            )  # fake the starttime and endtime
        else:
...
print(
    "[+] {} tasks processed. Attempting to generate schedule.".format(mgr.num_tasks())
)

prompted = False
while MAX_HOURS < 24:
    try:  # we often expect this to fail so using an exception is a little worse than returning None
        agenda = mgr.calc_agenda(
            MAX_HOURS,
            max_days=MAXIMIZE_DAYS,
            work_days=WORK_DAYS,
            work_today=WORK_TODAY,
            user_specified_days=USER_SPEC_DAYS,
        )
        break
        ...
        if not prompted:
            print(
                "[!] Not enough time available to finish your work in {} hours, increasing as necessary.".format(
                    MAX_HOURS
                )
            )
            prompted = True
...
print(
    "[+] Schedule successfully generated with max {} hours per day.".format(MAX_HOURS)
)
for i, plan in enumerate(agenda[:7]):
    if len(plan) > 0:
        ...
        print("")
        print(
            "{}, {} (work {} hours total):".format(
                week_index_to_day(day.weekday()), day, hours_total
            )
        )
        final = [
            [
                task.description,
                "{}/{}".format(hours, task.hours),
